...
-- Marque con E los constructores para que no se mezclen con otros constructores pero se puede quitar
  ...
  | ESqrt Expr
  | EExpt Expr
  -- Operaciones booleanas (Se pueden cambiar nombres)
  | ENot Expr 
  | EEq Expr -- igualdad
  | ELt Expr  -- menor que
  | EGt Expr -- mayor que         
  | ELe Expr -- menor o igual
  | EGe Expr -- mayor o igual
  | ENeq Expr -- distinto
  -- Pares
  | EPair Expr Expr -- Falta en la gramtica
  | EFst Expr
  | ESnd Expr
  -- Variables ligadas 
  | ELet Var Expr Expr
  | ELetrec Var Expr Expr
  | ELetStar [(Var, Expr)] Expr -- let*
  -- Condicionales
  | EIf0 Expr Expr Expr
  | EIf Expr Expr Expr
  | ECond [(Expr, Expr)] -- Dudas con este
  -- Funciones
  | ELambda Var Expr
  | EApp Expr Expr
  --Listas
