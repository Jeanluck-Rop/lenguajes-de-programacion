...
Citando al profesor, en su archivo compartido \textbf{\textit{Especificación Formal de los Lenguajes de Programación. Sintaxis Concreta}}~\cite{EFLP-SC}.
...
%%% Corrección 1 %%%%
  \begin{tcolorbox}[title=\textbf{Corrección 1: Diferencia entre predicados y comparaciones}, colframe=red!75!black, colback=red!5!white]
    \begin{itemize}
    \item \textbf{Comparaciones}:  
    Son expresiones cuyo propósito es evaluar una relación entre dos valores, típicamente numéricos.  
    Ejemplos característicos incluyen \texttt{<}, \texttt{>}, \texttt{<=}, \texttt{>=}, \texttt{=}.  
    Estas operaciones devuelven un valor booleano, pero su propósito principal es \emph{evaluar una relación matemática}.

    \item \textbf{Predicados}:  
    Un predicado es cualquier función que \textbf{produce un valor booleano} como resultado.  
    Pueden implementar comparaciones, pero también pueden realizar verificaciones más complejas, como:  
    En nuestro caso en especial por predicados nos referimos especificamente al \texttt{not}.
    \end{itemize}

En resumen, \textbf{todas las comparaciones son predicados}, pero \textbf{no todos los predicados son comparaciones}.  
Las comparaciones se limitan a relaciones matemáticas entre valores; los predicados abarcan cualquier función cuyo resultado sea un booleano.
\end{tcolorbox}

  \item \textbf{Asignaciones (aplicación de funciones) y funciones anónimas.}
...
deben procesar. Esto se logra mediante un \textbf{Análisis léxico} y un \textbf{Análisis sintáctico}\cite{ref2}.

%2.1 Sintaxis léxiva listo %%%%%%%
...
En pocas palabras, citando algo mencionado por profesor en clase:
\begin{quote}
...
aquí definidas\cite{ref2}.

%%%%%% 2.1.1 Analisis Léxico %%%%%

\subsection{Análisis Léxico}
Nuestra sintaxis se constituye apartir de un \textbf{análisis léxico}. El análisis léxico constituye la fase inicial en
...
\begin{quote}
  Definimos una función $\texttt{lexer:} \sum^* \rightarrow \texttt{[}Token\texttt{]}$, que toma una cadena de caracteres y produce la lista de $tokens$ según las expresiones regulares que hayamos definido en nuestro lenguaje.\cite{ref4}
\end{quote}
...
\begin{itemize}
  \item \textbf{Paréntesis:} \textbf{(} y \textbf{)},con los que indicamos cuando comienzan y terminan nuestras expresion (por eso se llaman $delimitad6ores$).
...
%%%% Corrección 2 %%%%%
\begin{tcolorbox}[title=\textbf{Corrección 2 - Tokens usando notación Regex}, colframe=red!75!black, colback=red!5!white]

\begin{itemize}
  \item \textbf{Paréntesis:} \verb+[\(|\)]+, son los delimitadores de expresiones.
  \item \textbf{Variables:} \verb+[a-zA-Z][a-zA-Z0-9]*+.
  \item \textbf{Números enteros:} \verb!-?[0-9]+!.
  \item \textbf{Booleanos:} 
    \begin{itemize}
      \item \verb+\#t|\#f|not+
    \end{itemize}
  \item \textbf{Operadores aritméticos:} 
    \begin{itemize}
      \item \verb![+|-|*|\|add1|sub1|sqrt|**]!
    \end{itemize}
  \item \textbf{Predicados y comparaciones:} \verb+[=|!=|<=|>=|<|>]+.
  \item \textbf{Asignaciones y funciones:} \verb+(let|let*|letrec|lambda)\b+
  \item \textbf{Pares ordenados y proyecciones:} 
    \begin{itemize}
      \item \texttt{(first|second)} (proyecciones)
      \item \texttt{,} (separador de pares)
    \end{itemize}
  \item \textbf{Condicionales:} \verb+(if|if0|cond)\b+.
  \item \textbf{Listas:} \textbf{Corchetes:} \verb+[\[|\]|,|head|tail]+ que son los delimitadores de listas además sus elementos están separados por comas \texttt{,}, junto con operaciones básicas \texttt{head} y \texttt{tail}.
\end{itemize}
\end{tcolorbox}
...
%%%%%%% Corrección 3 %%%%%%
\begin{tcolorbox}[title=\textbf{Corrección 3: Corrección del parrafo sobre Tokens },colframe=red!75!black, colback=red!5!white]
La estructura del tipo \textit{Token} son las piezas fundamentales del lenguaje, tales como paréntesis, números enteros, booleanos operadores aritméticos, palabras reservadas.Esto nos permite construir la sintaxis del
lenguaje de manera estructurada y libre de ambigüedades. Además, los \textit{Token} facilitan el trabajo del parser, ya que convierten el texto en una secuencia de unidades léxicas claramente clasificadas, lo que permite aplicar las reglas sintácticas sin analizar caracteres individuales y reduce significativamente la complejidad del análisis.
...
eficiente. Esta elección se fundamenta en varias ventajas significativas \cite{ref6}:
...
Después, definimos los patrones básicos que establecen los bloques fundamentales para construir patrones más complejos, promoviendo la posibilidad de reutilizar el código y claridad. Estas líneas no son código Haskell, sino instrucciones para Alex, con ellos le indicamos a Alex: ``\textit{Cuando veas \$digit en las reglas, reemplázalo por 0-9}''. Lo
...
\begin{itemize}
\item \textbf{Expresión regular:} Es la secuencia de caracteres que el lexer debe reconocer. En este
  caso, se trata de los símbolos estructurales o palabras reservadas como \texttt{(}, \texttt{let}, \texttt{+}, etc.
...
Es importante resaltar el caso de los \textbf{operadores} \texttt{let*}, \texttt{letrec},
...
longitud, prevalece la primera regla definida en el archivo\cite{ref6 - ref2}.\\
...
Con la notación \textbf{EBNF} \cite{iso14977} :
\begin{itemize}
...
\begin{tcolorbox}[title=\textbf{Corrección 4 - Notación EBNF y justificación de su uso}, colframe=red!75!black, colback=red!5!white]
En este proyecto utilizamos la notación EBNF conforme al estándar \texttt{ISO/IEC 14977:1996(E)}, en el que se establece lo siguiente: 
\begin{itemize}
\item \texttt{::=} para las reglas de producción.
\item \texttt{<...>} para los no terminales.
\item \texttt{{...}} para indicar repetición de cero o mas elementos.
\item \texttt{[...]} para indicar opcionalidad.
\item \texttt{|} para alternativas.

Elegimos este estándar porque es ampliamente utilizado en especificación formal de lenguajes de programación y coincide con la notación empleada en las notas de clase y en las herramientas académicas del curso (Alex, Happy y la gramática de MINILISP). Su expresividad permite describir la sintaxis de MINILISP de forma precisa, compacta y sin ambigüedades.
\end{itemize}

\end{tcolorbox}

De esta manera, cada producción de la gramática define cómo los \textit{tokens} generados por el analizador
léxico (como \texttt{TokenAdd}, \texttt{TokenIf}, \texttt{TokenLet}, etc.) se combinan para formar expresiones válidas en el lenguaje. Recordemos que en la sección de \textbf{sintaxis léxica} se estableció la correspondencia entre patrones de texto y sus respectivos tokens; ahora, en esta etapa, esos mismos tokens se convierten en los símbolos terminales de nuestra gramática.\\
...
%%%%%%%
\begin{tcolorbox} [title=\textbf{Corrección 5: Aclaración sobre los corchetes y su papel en la gramática}, colframe=red!75!black, colback=red!5!white]
En esta producción, los símbolos \texttt{"["} y \texttt{"]"} \textbf{sí son terminales} del lenguaje MINILISP. Se escriben entre comillas para distinguirlos de los corchetes usados por la notación EBNF, donde \texttt{[\,]} indica opcionalidad. 

Por tanto:
\begin{itemize}
    \item \texttt{"["} y \texttt{"]"} representan \emph{los corchetes literales} que el usuario escribe en la sintaxis concreta para formar listas.
    \item Los corchetes sin comillas \texttt{[\,]} siguen siendo parte de la notación EBNF, e indican que el contenido es opcional.
\end{itemize}

Además vamos a definir de manera no ambigüa \texttt{<Var>} y \texttt{<Num>}:
\begin{verbatim}
\texttt{<Var>} ::= \texttt{[A-Za-z][A-Za-z0-9]*
\texttt{<Num>} ::= \texttt{-?[0-9]+}
\end{verbatim}
...
multiplicación y división) que son variádicos, efectivamente lo sean. Decidimos forzar que cada uno de ellos reciba al menos dos expresiones, ya que el uso de las llaves \texttt{{}} en la notación \textbf{EBNF} indica que puede haber cero o más repeticiones. Por ello, cualquier invocación de un operador aritmético con menos de dos operandos no será aceptada por el lenguaje.\\
...
establecimos que debe haber exactamente un par \texttt{(\nt{Var} \nt{Expr})} como mínimo, permitiendo además la inclusión de
múltiples pares adicionales. Así reflejamos la posibilidad de definir una o más asociaciones dentro de un mismo bloque, manteniendo la flexibilidad solicitada para el proyecto.

Similarmente con la aplicación de funciones y las funciones $\lambda$, donde especificamos que debe haber al menos una variable para la función lambda y  exactamente dos expresiones expresiones para la aplicación de función: donde la
...
la sintaxis concreta del lenguaje (las listas y condicionales \texttt{cond}), mientras que los corchetes sin comillas siguen indicando opcionalidad en la notación formal. Así, la regla:
...
En otras palabras, el analizador sintáctico organiza los tokens generados por el \texttt{lexer} conforme a las producciones de la gramática, construyendo una representación jerárquica del programa. Esta representación se
denomina \textbf{árbol de sintaxis abstracta} (\textit{ASA} o \textit{Abstract Syntax Tree}-\textbf{AST}), el cual captura la estructura lógica del programa, eliminando detalles superficiales como los paréntesis o separadores que solo sirven para dar forma a la sintaxis concreta \cite{ref4}.\\
...
El análisis sintáctico representa entonces, una etapa intermedia y esencial dentro del proceso de interpretación: traduce la estructura lineal de los tokens en una forma jerárquica que puede ser fácilmente interpretada y
evaluada por etapas posteriores de\minilisp\hspace{-0.2cm}.

\bigskip

\noindent
Con todo lo visto en este capítulo, podemos concebir la \textit{\textbf{Sintaxis Concreta}} de nuestro lenguaje como la composición funcional entre el \textbf{analizador léxico} y el \textbf{analizador sintáctico}, donde
...
(\textit{Árbol de Sintaxis Abstracta}) es la estructura resultante. Y con esto cubrimos las fases que onforman el puente entre la entrada textual del usuario y las representaciones internas que permiten la evaluación del
lenguaje.\\

\noindent
A partir de este punto, continuaremos con las definiciones formales que dan estructura interna a nuestro lenguaje \minilisp\hspace{-0.2cm}, entramos en el tema de la construcción del \textbf{árbol de sintaxis abstracta} y la implementación del \textbf{analizador sintáctico} (\textit{parser}) usando Happy.
