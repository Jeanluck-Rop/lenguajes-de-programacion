...
La formalización de la semántica permite razonar rigurosamente sobre programas, verificar propiedades (por ejemplo corrección y seguridad), y relacionar la especificación formal con una implementación (intérprete o compilador). Existen distintos enfoques para dar semántica a un lenguaje (operacional, denotacional y axiomática)\cite{M06}. En este trabajo nos centraremos en la semántica operacional y, concretamente, en el estilo \emph{small-step} (semántica estructural), que es el más conveniente para \minilisp por su claridad al exponer transformaciones intermedias.
...
\begin{tcolorbox}[title=\textbf{Corrección 31: }, colframe=red!75!black, colback=red!5!white]
La semántica operacional especifica el significado de los programas describiendo la computación como una secuencia de pasos en una máquina abstracta. A diferencia de la semántica denotacional, que asigna directamente un objeto matemático estático al programa, el enfoque operacional estructural (Small-step) define un sistema de reglas de inferencia que formalizan las transiciones de estado individuales. Cada regla describe cómo una expresión se transforma (o reduce) en una más simple, avanzando desde un estado inicial hasta un valor canónico o error\cite{PierceBC}.

Este enfoque resulta especialmente adecuado para lenguajes funcionales como MiniLisp por dos razones fundamentales:
\begin{itemize}
\item Modelado de la reducción: Al basarse en el paradigma funcional, la ejecución en MiniLisp es esencialmente un proceso de reescritura de términos (reducciones $\beta$ y sustituciones). La semántica de paso pequeño captura explícitamente estas transformaciones intermedias, lo que permite visualizar la dinámica de las funciones recursivas y la aplicación de clausuras.
\item Correspondencia con la implementación: Al definir la evaluación como una serie de pasos discretos, las reglas semánticas se traducen casi directamente en la función de transición del intérprete (en nuestro caso, la función pasito), facilitando la verificación de que el código implementado es fiel a la especificación formal.
\end{itemize}
\end{tcolorbox}
...
\begin{tcolorbox}[title=\textbf{Corrección 32: Confusión en la atribución histórica de los autores y los estilos semánticos $Gran error pido disculpas$}, colframe=red!75!black, colback=red!5!white]
Este estilo fue introducido por Gilles Kahn en 1987 bajo el nombre de Semántica Natural. A diferencia del enfoque estructural de Plotkin, la semántica natural relaciona directamente un término con su valor final, ignorando los estados intermedios\cite{kahn1987N}.
\end{tcolorbox}
...
\begin{tcolorbox}[title=\textbf{Corrección 33:  }, colframe=red!75!black, colback=red!5!white]
Este enfoque fue sistematizado por Gordon Plotkin como parte del marco \textit{Structural Operational Semantics} (SOS)~\cite{Plotkin}. 
Si bien este marco es aplicable a diversos paradigmas de programación, su adopción para \minilisp se justifica porque permite formalizar explícitamente la \textbf{estrategia de reducción}. 
Al definir la evaluación mediante pasos discretos de reescritura, podemos especificar con precisión el orden de evaluación (en nuestro caso, estricto de izquierda a derecha) y distinguir claramente entre estados intermedios, errores de ejecución y no-terminación, detalles que una semántica de paso grande (\textit{Big-step}) tiende a abstraer.
\end{tcolorbox}
...
donde \(e_0 \in I\), cada transición \(e_i \rightarrow e_{i+1}\) está justificada por una regla de evaluación, y \(e_n \in F\) representa un estado final\cite{Plotkin}.
...
\begin{tcolorbox}[title=\textbf{Corrección 34:  }, colframe=red!75!black, colback=red!5!white]
La función \texttt{toFinalState} realiza la inyección del dominio sintáctico al dominio semántico. Aunque existe una correspondencia estructral entre la mayoría de los constructores del AST y del ASV, esta transformación es indispensable por dos razones teóricas:
\begin{enumerate}
\item \textbf{Uniformidad en la Transición:} La semántica de paso pequeño se define como una relación $e \to e'$ sobre un mismo conjunto. Al elevar la sintaxis a ASV, garantizamos que tanto las expresiones no evaluadas como los valores parcialmente reducidos pertenezcan al mismo tipo de dato, permitiendo que la función de transición pasito sea endomórfica ($\text{ASV} \to \text{ASV}$).
\item \textbf{Valores no denotables en la sintaxis:} El tipo ASV es estrictamente mejor que el AST, pues incluye constructores como Closure que representan valores en tiempo de ejecución (funciones junto a su entorno) y que no tienen representación textual directa en el código fuente original.\end{enumerate}
...
\begin{tcolorbox}[title=\textbf{Corrección 35: Definición de conjunto de total de estados  }, colframe=red!75!black, colback=red!5!white]
Sea $E$ el conjunto de todos los estados posibles, definido como $E = \text{ASV} \times \text{Env} \cup \{\textbf{Error}\}$.
\end{tcolorbox}
...
\item Expresiones atómicas:
  ...
  \item VarV(i):
    ...
    \]
    \[
    \frac{lookup\ i\ \varepsilon \ \text{no está definido}}{\langle Varv(i), \varepsilon \rangle \to \text{Error}}
    \]

\begin{tcolorbox}[title=\textbf{Corrección 36: VarV(i)S }, colframe=red!75!black, colback=red!5!white]
\item VarV(i):
    \[
    \frac{lookup(i, \varepsilon) = v}{\langle VarV(i), \varepsilon \rangle \to \langle v, \varepsilon \rangle} \quad (\text{E-Var})
    \]
    \[
    \frac{lookup(i, \varepsilon) = \bot}{\langle VarV(i), \varepsilon \rangle \to \textbf{Error}} \quad (\text{E-VarErr})
    \]
\end{tcolorbox}
...
\begin{tcolorbox}[
    title=\textbf{Corrección: Reglas Definición de conjunto de total de estados},
    colframe=red!75!black,
    colback=red!5!white,
    fonttitle=\bfseries
]
Las correcciones de las reglas se hicieron directamente en el texto sin agregar cajita.
\end{tcolorbox}
  ...
  \item NiV:
    \[
    \frac{}{\langle NiV, \varepsilon \rangle \to \langle NiV, \varepsilon \rangle}
    \]
  \end{itemize}
  
\item AddV(i,d):
    \textit{Caso 1: Evaluar lado izquierdo (Left)}
  \[
  ...
  \]
  \textit{Caso 2: Evaluar lado derecho (Right)}
  \[
  ...
  \]
  \textit{Caso 3: Realizar la operación (Operation)}
  \[
  \frac{}{\langle AddV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle NumV(n +m), \varepsilon \rangle}
  \]
...
\item SubV(i,d):
  ...
  \]
  \[
  \frac{}{\langle SubV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle NumV(n - m), \varepsilon \rangle}
  \]
...
\item MulV(i,d):
  ...
  \]
  \[
  \frac{}{\langle MulV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle NumV(n *m), \varepsilon \rangle}
  \]
...
\item DiV(i,d):
  ...
  \]
  \[
  \frac{}{\langle DiV(NumV(n),NumV(0)), \varepsilon \to \text{Error}}
  \]
  \[
  \frac{}{ \langle DiV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle NumV(n / m), \varepsilon \rangle} \quad (m \neq 0)
  \]
...
\item SqrtV(n):
  ...
  \]
  \[
  \frac{n < 0}{\langle SqrtV(NumV(n)), \varepsilon \to \text{Error}}
  \]
  \[
  \frac{}{\langle SqrtV(NumV(n)), \varepsilon \rangle \to \langle NumV(\sqrt{n}), \varepsilon \rangle}  \quad (n \geq 0)
  \]
...
\item EqualV(i,d):
  ...
  \]
  \[
  \frac{}{\langle EqualV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle BoolV(n = m), \varepsilon \rangle}
  \]
...
\item LessV(i,d)
  ...
  \]
  \[
  \frac{}{\langle LessV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle BoolV(n < m), \varepsilon \rangle}
  \]
...
\item GreaterV(i,d)
  ...
  \]
  \[
  \frac{}{\langle GreaterV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle BoolV(n > m), \varepsilon \rangle}
  \]
...
\item LeqV(i,d)
  ...
  \]
  \[
  \frac{}{\langle LeqV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle BoolV(n \leq m), \varepsilon \rangle}
  \]
...
\item GeqV(i,d)
  ...
  \]
  \[
  \frac{}{\langle GeqV(NumV(n),NumV(m)), \varepsilon \rangle \to \langle BoolV(n \geq m), \varepsilon \rangle}
  \]
\end{itemize}

Como se puede ver en las reglas para los comparadores, la regla final cuando ambas expresiones del comparador son el resultado deriva en un estado final \texttt{BoolV}, el cual captura el resultado lógico de la comparación (verdadero o falso) entre los dos enteros evaluados.
...
\item IfV(c,t,e), IfV solo evalúa la condicional hasta llegar a un \texttt{BoolV} mas no evalúa el \textbf{then} o \textbf{else} dependiendo del resultado de la condición, solo se encarga de retornar alguno de los dos dependiendo del caso:
  ...
  \]
  \[
  \frac{}{\langle IfV(BoolV(True),t,e), \varepsilon \rangle \to \langle t, \varepsilon \rangle}
  \]
  \[
  \frac{}{\langle IfV(BoolV(False),t,e), \varepsilon \rangle \to \langle e, \varepsilon \rangle}
  \]
...
\item HeadV(l):
  \[
  \frac{\langle l, \varepsilon \rangle \to \langle l', \varepsilon \rangle}{\langle HeadV(l), \varepsilon \rangle \to \langle HeadV(l'), \varepsilon \rangle}
  \]
  \[
...
\item TailV(l);
  \[
  \frac{\langle l, \varepsilon \rangle \to \langle l', \varepsilon \rangle}
       {\langle TailV(l), \varepsilon \rangle \to \langle TailV(l'), \varepsilon \rangle}
  \]

  \[
  \frac{isCons(v_t) \quad \langle v_t, \varepsilon \rangle \to \langle v_t', \varepsilon \rangle}
       {\langle TailV(ConV(v_h, v_t)), \varepsilon \rangle \to \langle TailV(ConV(v_h, v_t')), \varepsilon \rangle}
  \]

  \[
  \frac{\neg isCons(v_t)}
       {\langle TailV(ConV(v_h, v_t)), \varepsilon \rangle \to \langle v_t, \varepsilon \rangle}
  \]
...
\item AppV(f,a):
  \[
  \frac{\langle f, \varepsilon \rangle \to \langle f', \varepsilon \rangle}
       {\langle AppV(f,a), \varepsilon \rangle \to \langle AppV(f',a), \varepsilon \rangle}
  \]

  \[
  \frac{\langle a, \varepsilon \rangle \to \langle a', \varepsilon \rangle}
       {\langle AppV(Closure(p, c, \varepsilon'), a), \varepsilon \rangle \to \langle AppV(Closure(p, c, \varepsilon'), a'), \varepsilon \rangle}
  \]

  \[
  \frac{v_a \text{ es valor}}
       {\langle AppV(Closure(p, c, \varepsilon'), v_a), \varepsilon \rangle \to \langle c, \varepsilon'[p \mapsto v_a] \rangle}
  \]
...
\begin{tcolorbox}[
    title=\textbf{Corrección 38: Conclusión},
    colframe=red!75!black,
    colback=red!5!white,
    fonttitle=\bfseries
]
En resumen, las reglas de evaluación proporcionan el marco formal necesario para describir con precisión el proceso de ejecución de los programas dentro de un lenguaje. 
A través de estas reglas se establece, de manera inductiva, cómo las expresiones evolucionan paso a paso bajo un ambiente de evaluación, garantizando un comportamiento bien definido y verificable. 
Esto es posible gracias al \textbf{determinismo} del sistema de transición, el cual asegura que para cualquier configuración válida existe, una única transición posible o un valor final, permitiendo trazar la ejecución mediante árboles de derivación únicos.

Este enfoque no solo facilita la implementación y el razonamiento formal del lenguaje, sino que también provee el mecanismo para distinguir entre estados válidos y fallos en tiempo de ejecución.
Aunque en esta especificación nos hemos centrado en formalizar explícitamente errores críticos como la \textbf{división por cero} y el acceso a \textbf{variables libres}, el marco semántico es extensible para capturar otros comportamientos anómalos (como errores de tipo dinámicos) mediante la definición de nuevos estados de error o estados bloqueados.
\end{tcolorbox}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
  ...
  \item Aplicar las reglas de evaluación de manera iterativa (paso a paso), hasta alcanzar un valor final (estado final), una expresión bloqueada (configuración para la cual no existe ninguna regla de transición definida), o un error (estado al que el sitema va cuando detecta una operación inválida).
...
La función \texttt{eval} recibe un estado final con el ambiente de inicio el cuál es vacío al comienzo de la evaluación y devuelve un \texttt{ASV} que se avalúa recursivamente y al terminar será un valor canónico.
...
Modelamos las reglas de evaluación con paso pequeño de\minilisp\hspace{-0.2cm} con la función \texttt{pasito}. Esta función \texttt{pasito} recibe el estado final con su ambiente a evaluar y regresa la configuración resultante:
...
Tenemos la mención especial de que para las variables \texttt{VarV} utilizamos \texttt{lookup}, que en nuestra implementación, con un arrebato increíble de originalidad lo nombramos como \texttt{mirarriba}; para buscar las variables en el ambiente dado. La función implementa la búsqueda de una variable en el ambiente, toma el nombre de una variable y un ambiente \texttt{Env} y devuelve el \texttt{ASV} asociado.
...
Para el caso de \texttt{HeadV} con \texttt{ConV} es análogo a \texttt{FstV} con los pares. Sin embargo, \texttt{TailV} no solo necesita que ambos argumentos ya sea los valores, sino que el segundo valor no sea de tipo \texttt{ConV}, pues esto significa que hay más expresiones en el encadenamiento de \texttt{ConV}, por lo que necesitamos seguir con este segundo argumento hasta llegar a un valor canónico dinstinto de \texttt{ConV} (justo como en las reglas de evaluación para esta expresión).
...
\begin{itemize}
  \item el combinador \textbf{Z} está diseñado para lenguajes con evaluación perezosa\cite{N15},
