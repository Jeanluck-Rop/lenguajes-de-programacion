...
Continunado con nuestro proyecto. Una vez que hemos elimindo todos los elementos superficilaes y auxiliares de la sintaxis concreta y léxica para quedarnos únicamente con la represntación mínima y estructural del programa, es decir, el \textbf{Arbol de Sintaxis Abstracta} (ASA) logramos capturar la estructura esencial del programa. 
...
Pasando al caso específico de \texttt{letrec}, este nos permite definir funciones recursivas locales. Formalmente, la recursión puede desazucararse utilizando un operador de punto fijo. Para lenguajes de evaluación ansiosa se utiliza el operador Z, que implementa recursión sin necesidad de auto-referencia explícita.
...
=======
Por otro lado, \texttt{LetRec} como su nombre indica, necesita implementar recursión. 

\begin{quote}
  \textit{Aqui nos extendemos con la explicacion de los combinadores y sobre todo el combinar Z}
\end{quote}

Queda desacuzarado como sigue:
...
\[
\text{let* } \left[ \left(i,\, (\lambda i.\, v)\,(\lambda i.\, v)\right) \right]\, b
\]
...
\bigskip
=======
Donde la variable \texttt{Var "Z"} queda definida con el combinador \texttt{Z}:
\[
\texttt{(lambda (f) ((lambda (x) (f (lambda (v) ((x x) v)))) (lambda (x) (f (lambda (v) ((x x) v)))))) metida en el ambiente}
\]
