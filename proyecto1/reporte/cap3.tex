...
Está enfocada en capturar la \textbf{lógica} y \textbf{jerarquía} del programa dejando de lado elementos puramente sintácticos como los paréntesis o el formato. Formalizar en ella nos permitirá desarrollar un intérprete más eficiente y robusto, ya que nos facilita la detección de errores, la optimización del código y de incorporar en un futuro nuevas funcionalidades sin mayor problema. Justo como en todo nuestro campo de trabajo,
...
analizadores, optimizadores e intérpretes\cite{HyU}.
...
construcción del lenguaje, y las hojas representan valores o identificadores \cite{HyU}.
...
Con lo anterior establecido, necesitamos ahora definir formalmente las reglas que nos permitan especificar los
...
identificarlos como tales, tendrán su etiqueta. Pues aunque no tengas estas palabras reservadas, se disntinguen por
...
de\minilisp\hspace{-0.2cm} y definimos la descripción de sus reglas de formación de las expresiones en la sintaxis abstracta (ASA):
...
Las reglas anteriores podemos considerarlas como los \textit{nodos hoja} de\minilisp\hspace{-0.2cm} (no requieren descomponerse en subexpresiones).

\begin{tcolorbox} [title=\textbf{Corrección 6: aclaración sobre el uso de tipos de Haskell}, colframe=red!75!black, colback=red!5!white]

El uso de los tipos \texttt{String}, \texttt{Int} y \texttt{Bool} no implica que la definición de la 
sintaxis abstracta de nuestra implementación de MINILISP dependa del lenguaje anfitrión Haskell. Estos nombres se emplean 
únicamente como notación conveniente para referirnos, respectivamente, a cadenas, números enteros 
y valores booleanos del lenguaje MINILISP. 

La sintaxis abstracta del lenguaje es independiente del lenguaje de implementación; las expresiones 
atómicas \texttt{Var(s)}, \texttt{Num(n)} y \texttt{Boolean(b)} se definen por sus propiedades en 
MINILISP, no por los tipos del entorno de programación utilizado para construir el intérprete.
...
\begin{tcolorbox} [title=\textbf{Corrección 7: Sobre citar el formato}, colframe=red!75!black, colback=red!5!white]

La notación utilizada en esta parte del proyecto se basa principalmente en las notas de clase del profesor\cite{ref4} Esta misma notación se puede encontrar en textos fundamentales como Types and Programming Languages de Pierce \cite{PierceBC} y Compilers: Principles, Techniques, and Tools \cite{ref2}.
...
manera en que se manejan los ambientes y la aplicación de funciones dentro de nuestro lenguaje\minilisp\hspace{-0.2cm}.\\
...
En particular:
\begin{itemize}
\item \texttt{let} define un nuevo ambiente donde las variables se evalúan en paralelo (\textbf{alcance estático}).
...
\begin{tcolorbox}[title=\textbf{Corrección 8: aclaración del término “valor” en Let, LetRec y LetStar}, colframe=red!75!black, colback=red!5!white]

En las reglas anteriores el término “valor” fue un uso impreciso. En la sintaxis abstracta de 
MINILISP, los argumentos \texttt{v}, \texttt{v\_i} o \texttt{v\_j} de los constructores 
\texttt{Let}, \texttt{LetRec} y \texttt{LetStar} pueden ser \textbf{cualquier expresión ASA}, 
no necesariamente un valor ya evaluado. 

Por lo tanto, estas reglas deben entenderse como:

\begin{itemize}
    \item \texttt{LetRec(i, v, b)} es un ASA si \texttt{i} es un identificador y 
    \texttt{v} y \texttt{b} son expresiones ASA.
    \item \texttt{Let((i\_1, v\_1), …, (i\_n, v\_n), b)} es un ASA si cada 
    \texttt{i\_k} es un identificador y cada \texttt{v\_k} y \texttt{b} son expresiones ASA.
    \item Lo mismo aplica para \texttt{LetStar}.
\end{itemize}

El uso anterior de la palabra “valor” no debe interpretarse como una restricción semántica en 
la sintaxis; únicamente se quiso indicar que \texttt{v} y \texttt{v\_i} son componentes 
expresivos, pero no necesariamente valores finales.
...
dividen en: \textbf{variables}, \textbf{abstracciones $\lambda$} y la aplicación de funciones\cite{ref9}.\\

Nuestras funciones \texttt{Lambda} se representan en el \textbf{ASA} como una lista de encabezados y una
expresión que constituye el cuerpo de la función, que vendrían siendo las \textbf{variables} y
...
\begin{enumerate}
\item Como explicamos, la primer expresión representa la función que será aplicada (una expresión \texttt{Lambda} en nuestro lenguaje).
...
\item \textbf{Expresiones condicionales.}
  ...
  \]
  \[
  (\texttt{cond [}(\texttt{> x 0})\texttt{ 1] [}(\texttt{< x 0})\texttt{ 2] [else 3]})
  \]
  \[
  \Rightarrow
  \]
  \[
  \texttt{Cond [}(\texttt{Greater [Var "x", Num 0], Num 1}) (\texttt{Less [Var "x", Num 0], Num 2})] (\texttt{Num 3})
  \]
...
\item \textbf{Funciones y aplicación.}
  ...
  \[
  \hspace{-1.8cm}
  ((\texttt{lambda} (\texttt{x y}) (\texttt{+ x y})) \texttt{2 3})
  \]
  \[
  \Rightarrow
  \]
  \[
  \texttt{App} (\texttt{Lambda [``x'', ``y'']} (\texttt{Add [Var ``x'', Var ``y'']}) \texttt{[Num 2, Num 3]})
  \]
...
\begin{tcolorbox}[title=\textbf{Corrección 9: Aclaración sobre la notación $\Rightarrow$ }, colframe=red!75!black, colback=red!5!white]
La notación $\Rightarrow$ utilizada en los ejemplos no debe interpretarse como una regla formal de 
reescritura ni como una transformación sintáctica completa. Su propósito es únicamente 
ilustrar la correspondencia entre una expresión concreta y su representación en la 
Sintaxis Abstracta (ASA). 

En una implementación real, la traducción desde la sintaxis concreta hacia el ASA es un 
proceso \textbf{recursivo y composicional}: cada subexpresión se transforma de manera independiente 
y luego se combinan sus resultados. Esto se realiza en el código, pero no se especificó 
formalmente en esta sección.

En particular, el constructo \texttt{SugarAdd} no fue introducido como un constructor 
variádico propio; cualquier azúcar sintáctica de este tipo se descompone automáticamente 
en aplicaciones del constructor \texttt{Add} durante la fase de construcción del ASA. Por ello, 
las expresiones del estilo \texttt{(+ x)} o \texttt{(+ (+ 1 2) 3)} quedan representadas como 
estructuras anidadas de \texttt{Add}, sin que $\Rightarrow$ denote una transformación formal que deba 
definirse recursivamente en esta sección.
\end{tcolorbox}
...
similar a \textbf{BNF}, en un módulo de Haskell que implementa automáticamente el parser correspondiente\cite{ref7}.\\
...
En Happy, cada producción de la gramática se asocia con una acción semántica en Haskell, que construye el nodo correspondiente en el \textbf{ASA}. De este modo, Happy no solo valida la estructura del programa, sino que también construye automáticamente la representación estructural\cite{ref7}.\\
...
\item \texttt{opArgs}: permite operadores aritméticos con un número variable de argumentos.
  ...
  Luego, \texttt{reverse} corrige el orden de evaluación\cite{ref7}.
...
\bigskip
\begin{tcolorbox}[title=\textbf{Corrección 10: Justificación con citas}, colframe=red!75!black, colback=red!5!white]

La razón por la cual usamos \textit{recursión izquierda} en Happy es porque así podemos definir las reglas de producción de forma más eficiente, ya que construimos el resultado del parser en una sola pasada utilizando espacio constante en la pila. Como señala \cite{ref2}, las gramáticas left-recursive son preferidas en el parsing bottom-up porque permiten un uso más eficiente de la pila, evitando el desbordamiento que puede ocurrir con recursión derecha en secuencias largas. La documentación de Happy \cite{ref7} confirma que este enfoque maneja reglas left-recursive de manera eficiente usando espacio constante en la pila, mientras que la \textit{recursión por derecha} podría incrementar la complejidad de tiempo y memoria de manera significativa.

Por ello utilizamos \textit{recursión por izquierda} de la forma \texttt{\{ [\$2, \$1]\}} y \texttt{\{\$2 : \$1\}}, así construimos la lista de expresiones variádicas hacia la izquierda. Como explica \cite{ref6}, esta técnica de acumulación con seguida de \texttt{reverse} es una práctica común en programación funcional para construir listas eficientemente mientras se mantiene el orden correcto final. 
\end{tcolorbox}

\noindent
Y así, como hemos visto en el capítulo, el estudio y la construcción de la \textbf{sintaxis abstracta}
