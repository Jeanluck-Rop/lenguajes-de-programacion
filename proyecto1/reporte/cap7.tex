...
Primero mostramos los resultados individuales de cada fase del código implementado que le da vida a nuestro lenguaje\minilisp\hspace{-0.2cm}.
...
Importamos todos los módulos a usar en el proyecto. Definimos el combinador \textbf{Z} y aplicamos us evaluación como es requerido para la recursión en\minilisp\hspace{-0.2cm}.

\noindent
El punto de entrada pincipal del lenguaje es la función \texttt{main}, donde damos la bienvenida al usuario y comnzamos el intérprete interactivo de\minilisp\hspace{-0.2cm}. De este modo hacemos lo siguiente para correr el proyecto:
...
Ya con esto, veamos los resultados finales del lenguaje\minilisp\hspace{-0.2cm}:
...
Estas funciones nos sirven como ejemplos prácticos para probar las capacidades del lenguaje\minilisp\hspace{-0.2cm} y en especial, de su capacidades recursivas.
...
Su implementación en Haskell genera el código\minilisp\hspace{-0.2cm} correspondiente con \texttt{letrec}:
...
\[
\texttt{(letrec (fibo (lambda (n)}
\]
\[
\quad \quad \texttt{(if0 n 0 (if0 (- n 1) 1 (+ (fibo (- n 1)) (fibo (- n 2))))))) (fibo 7))}
\]

Estas funciones de prueba no sólo demuestran la expresividad del lenguaje donde el sistema de evaluación de\minilisp\hspace{-0.2cm} maneja correctamente la recursión, sino también que la implementación del combinador \textbf{Z} y las expresiones \texttt{letrec} permiten definir y evaluar funciones complejas sin necesidad de estructuras externas. Además, muestran cómo el menú interactivo puede extenderse para admitir comandos personalizados que simplifican la interacción del usuario con el intérprete.
